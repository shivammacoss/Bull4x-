...
const Contact = () => {
  ...
  const [messages, setMessages] = useState([
    { from: 'agent', text: 'Hi there! 👋 I\'m Sarah from Bull4x Support. How can I help you today?', time: 'now' }
  ])
  ...
  const getAutoReply = (text) => {
    ...
    if (t.includes('account') || t.includes('open')) return 'You can open a free live or demo account in under 2 minutes from our Accounts page. Would you like me to send you the link?'
    if (t.includes('deposit') || t.includes('fund')) return 'We support card, bank wire, e-wallet, and crypto deposits with zero internal fees. Minimum deposit is $100 on Standard, $500 on Pro, and $2,000 on ECN.'
    if (t.includes('withdraw') || t.includes('payout')) return 'Most withdrawals are processed the same business day in USD, EUR, GBP, or USDT. Zero internal withdrawal fees on every live account.'
    if (t.includes('spread') || t.includes('commission') || t.includes('fee')) return 'Spreads start from 0.0 pips on ECN accounts ($3.5/lot/side), 0.6 pips on Pro (zero commission), and 1.2 pips on Standard (zero commission).'
    if (t.includes('platform')) return 'We offer MT4, MT5, our Bull4x Web Platform, plus Copy Trading and IB Management tools. Visit the Platforms page to learn more.'
    if (t.includes('regulat') || t.includes('license') || t.includes('safe')) return 'Bull4x is an authorised forex and CFD brokerage. Client funds are held in segregated tier-1 bank accounts with negative balance protection on every live account.'
    ...
    return 'Thanks for your message! One of our support specialists will get back to you shortly. In the meantime, feel free to ask about accounts, spreads, deposits, withdrawals, or platforms.'
  }
  ...
  const contactInfo = [
    {
      icon: Mail,
      title: 'Email Us',
      content: 'support@bull4x.com',
      link: 'mailto:support@bull4x.com'
    },
    {
      icon: Phone,
      title: 'Call Us',
            ...
            <ScrollReveal variant="fadeRight" delay={0.2}>
              <div>
                ...
                <Card className="p-8 mb-6">
                  <h3 className="text-xl font-semibold text-white mb-4">Bull4x Ltd</h3>
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-text-secondary">
                      <span className="text-white font-semibold">Email:</span> support@bull4x.com
                    </p>
                    <p className="text-text-secondary">
