 ...
 *
 * Bull4x datafeed — fast, no backend bar dependency.
 */
import type {
  Bar,
 ...
 *  Also tries a direct REST fetch as fallback so chart loads even before WebSocket connects. */
function waitForPrice(symbol: string, timeoutMs = 10000): Promise<{ bid: number; ask: number } | null> {
  const tick = useTradingStore.getState().prices[symbol];
  if (tick && tick.bid > 0) return Promise.resolve(tick);

  // Try REST API immediately in parallel with store subscription
  const restFetch = fetch(`/api/v1/instruments/${encodeURIComponent(symbol)}/price`)
    .then(r => r.ok ? r.json() : null)
    .then(d => d && d.bid > 0 ? { bid: Number(d.bid), ask: Number(d.ask) } : null)
    .catch(() => null);

  const storeListen = new Promise<{ bid: number; ask: number } | null>((resolve) => {
  ...
  // Return whichever resolves first with a valid price
  return Promise.any([
    restFetch.then(r => { if (!r) throw new Error('no price'); return r; }),
    storeListen.then(r => { if (!r) throw new Error('no price'); return r; }),
  ]).catch(() => null);
}
...
export const bull4xDatafeed: IBasicDataFeed = {
  onReady: (cb) => {
  ...
  searchSymbols: (
    ...
    const result: SearchSymbolResultItem[] = instruments
      ...
      .map((i) => ({
        ...
        exchange: 'Bull4x', ticker: i.symbol,
      ...
      if (!tick || tick.bid <= 0) {
        ...
        try {
          ...
          if (resp.ok) {
            ...
            const match = items.find((p: any) => (p.symbol || '').toUpperCase() === sym);
            if (match && Number(match.bid) > 0) {
              tick = { bid: Number(match.bid), ask: Number(match.ask) };
            }
          }
        } catch { /* ignore */ }
      }
