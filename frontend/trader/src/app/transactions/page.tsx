...
export default function TransactionsPage() {
  ...
  // Per-row formatter: each transaction carries its own currency (INR trades/
  // transfers settle in ₹), so format with that instead of the wallet default.
  const fmtCur = (n: number, cur?: string | null) => {
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <div className="text-right shrink-0">
                            ...
                            >
                              {isIn ? '+' : '-'}
                              {fmtCur(Math.abs(signed), tx.currency)}
                            </p>
                            <p
                              className={clsx(
