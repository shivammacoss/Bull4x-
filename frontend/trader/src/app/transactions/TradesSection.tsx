...
function closeReasonBadge(
  ...
  if (r === 'master_deleted')
    return { label: 'Master deleted', className: 'bg-warning/15 text-warning border border-warning/30' };
  if (r === 'algo_close')
...
export default function TradesSection() {
  ...
  // P&L/swap/commission are stored in the account currency, so pick the symbol
  // per row (₹ for INR accounts, $ for USD).
  const symFor = (id: string) => currencySymbol(accounts.find((a) => a.id === id)?.currency);
                      ...
                      >
                        ...
                        {tab !== 'pending' && (
                          <td className={clsx('px-3 py-3 text-right font-mono font-bold', pnl >= 0 ? 'text-buy' : 'text-sell')}>
                            {pnl >= 0 ? '+' : ''}{symFor(r.account_id)}{fmt2(pnl)}
                          </td>
                        )}
                        {tab === 'closed' && (() => {
      ...
      {selected && typeof document !== 'undefined' && createPortal(
        ...
        >
          ...
          >
            ...
            <TradeDetailBody kind={selected.kind} data={selected.data} accountNumber={accountNumber((selected.data as any).account_id)} sym={symFor((selected.data as any).account_id)} />
          </div>
        </div>,
        document.body,
      )}
...
function TradeDetailBody({ kind, data, accountNumber, sym = '$' }: { kind: TradeTab; data: any; accountNumber: string; sym?: string }) {
  ...
  if (kind !== 'pending') {
    ...
    items.push({ label: 'Swap', value: <span className="font-mono text-text-secondary">{sym}{fmt2(data.swap || 0)}</span> });
    items.push({ label: 'Commission', value: <span className="font-mono text-text-secondary">{sym}{fmt2(data.commission || 0)}</span> });
  }
  if (data.stop_loss) {
