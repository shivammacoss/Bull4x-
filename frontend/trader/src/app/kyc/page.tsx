...
/** Resize + re-encode KYC images on the client so a typical 3-5 MB phone photo
 *  uploads as a 400-600 KB JPEG. ID readability needs ~1600px on the long edge;
 *  anything bigger is just slower bytes. Non-images and small files pass through. */
...
export default function KycPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!address.trim() || !city.trim() || !country.trim()) {
      toast.error('Please fill residential address, city, and country');
      return;
    }

    // Optimistic: close the modal right away and run the upload in the
    // background. Phone photos are usually 2-5 MB so blocking the modal until
    // the network is done made it feel laggy even on good connections.
    const snapshot = { file, file2, docType, docType2, address, city, postal, country };
    setShowFormModal(false);
    const toastId = toast.loading('Submitting KYC…');

    try {
      // Compress in parallel — typically cuts a 3 MB phone photo to ~500 KB
      // and finishes in under a second, vs. 5-10 s saved on upload time.
      const [primary, secondary] = await Promise.all([
        compressImage(snapshot.file),
        snapshot.file2 ? compressImage(snapshot.file2) : Promise.resolve(null),
      ]);

      const fd = new FormData();
      fd.append('document_type', snapshot.docType);
      fd.append('file', primary);
      if (secondary) {
        fd.append('document_type_2', snapshot.docType2);
        fd.append('file_2', secondary);
      }
      fd.append('residential_address', snapshot.address.trim());
      fd.append('city', snapshot.city.trim());
      if (snapshot.postal.trim()) fd.append('postal_code', snapshot.postal.trim());
      fd.append('country_of_residence', snapshot.country.trim());
      ...
      // Restore the modal so the user can retry without re-entering everything.
      setShowFormModal(true);
    }
  };
          ...
          <div className="relative w-full sm:max-w-lg max-h-[92dvh] overflow-y-auto rounded-t-2xl sm:rounded-2xl border border-border-primary bg-card shadow-2xl sidebar-scroll">
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="space-y-3">
                <label className="text-xs text-text-secondary uppercase tracking-wide font-semibold">
                  Address <span className="text-sell">*</span> <span className="normal-case text-text-tertiary font-normal">(required)</span>
                </label>
                <input
                  type="text"
                  value={address}
