...
export default function SupportPage() {
  ...
  const fetchTicketDetail = useCallback(async (id: string, silent = false) => {
    ...
    try {
      ...
      if (!silent) toast.error(err instanceof Error ? err.message : 'Failed to load ticket');
  ...
  // Poll the active ticket so admin replies appear without a manual page refresh.
  // Skips when no ticket is selected and pauses while the tab is hidden to avoid
  // pointless requests; resumes immediately when the user re-focuses the window.
  ...
  useEffect(() => {
    ...
    const onFocus = () => fetchTicketDetail(selectedTicketId, true);

    const onVisible = () => { if (document.visibilityState === 'visible') fetchTicketDetail(selectedTicketId, true); };
  ...
  // Refresh the ticket list periodically too — so unread badges / status changes
  // (admin marking ticket resolved, new ticket created elsewhere) stay current.
