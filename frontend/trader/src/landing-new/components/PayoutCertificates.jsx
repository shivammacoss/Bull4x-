...
const certificates = [
  { id: 1, trader: 'Marcus Chen',     city: 'Singapore', amount: '$24,500', plan: 'Pro Account',      date: '12 Apr 2026', time: '14:32', txnId: 'IFX2604120143', bank: 'DBS Bank',         mode: 'Wire' },
  { id: 2, trader: 'Sofia Almeida',   city: 'Portugal',  amount: '$18,200', plan: 'Standard Account', date: '08 Apr 2026', time: '11:18', txnId: 'IFX2604080872', bank: 'USDT (TRC20)',    mode: 'Crypto' },
  { id: 3, trader: 'Hiroshi Tanaka',  city: 'Japan',     amount: '$31,800', plan: 'ECN Account',      date: '02 Apr 2026', time: '16:45', txnId: 'IFX2604020419', bank: 'MUFG Bank',       mode: 'Wire' },
  { id: 4, trader: 'Aisha Khan',      city: 'UAE',       amount: '$15,400', plan: 'Standard Account', date: '28 Mar 2026', time: '10:22', txnId: 'IFX2603281054', bank: 'Emirates NBD',    mode: 'Wire' },
  { id: 5, trader: 'Dimitri Volkov',  city: 'Estonia',   amount: '$42,100', plan: 'VIP Account',      date: '20 Mar 2026', time: '13:09', txnId: 'IFX2603200318', bank: 'SEB',             mode: 'SEPA' },
  { id: 6, trader: 'Elena Rodriguez', city: 'Mexico',    amount: '$27,650', plan: 'Pro Account',      date: '15 Mar 2026', time: '15:51', txnId: 'IFX2603150207', bank: 'USDT (ERC20)',    mode: 'Crypto' },
];
    ...
    <div className={`w-full h-full flex flex-col bg-white ${large ? 'p-8 sm:p-10' : 'p-4 sm:p-5'}`}>
      ...
      <div className={`flex items-center justify-between pb-3 mb-3 border-b border-[#E8EAF0] ${large ? 'pb-5 mb-5' : ''}`}>
        <div className="flex items-center gap-2">
          <div className={`rounded-md bg-[#0158c6] flex items-center justify-center text-white font-bold ${large ? 'w-9 h-9 text-sm' : 'w-7 h-7 text-[10px]'}`}>
            iFX
          </div>
          <div>
            <p className={`font-bold text-[#0D0F1A] leading-tight ${large ? 'text-sm' : 'text-[10px]'}`}>Bull4x</p>
            <p className={`text-[#6B7080] leading-tight ${large ? 'text-[10px]' : 'text-[8px]'}`}>Withdrawal Receipt</p>
          </div>
        </div>
      ...
      <div className={`pt-3 border-t border-dashed border-[#E8EAF0] flex items-center justify-between ${large ? 'mt-5' : 'mt-3'}`}>
        ...
        <p className={`text-[#0158c6] font-bold ${large ? 'text-[10px]' : 'text-[7px]'}`}>bull4x.com</p>
      </div>
    </div>
  );
}
    ...
    <section className="bg-white py-14 md:py-24 px-6">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-10 md:mb-14 max-w-3xl mx-auto">
          <p className="text-sm font-semibold text-[#0158c6] uppercase tracking-widest mb-4">Withdrawal Proofs</p>
          <h2
            className="font-extrabold text-[#0D0F1A] tracking-[-0.02em] font-manrope mb-4"
            style={{ fontSize: 'clamp(2rem, 4vw, 3rem)' }}
          >
            Real withdrawals to <span className="text-[#0158c6]">real global clients</span>
          </h2>
          <p className="text-base sm:text-lg text-[#6B7080]">
            Every receipt below is from a client who completed KYC verification
            and received their withdrawal in their preferred currency.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {certificates.map((c) => (
            <div
              key={c.id}
              className="group bg-white border border-[#E8EAF0] rounded-2xl overflow-hidden shadow-[0_2px_16px_rgba(0,0,0,0.04)] hover:shadow-[0_12px_40px_rgba(1,88,198,0.12)] hover:border-[#0158c6] transition-all duration-300 cursor-pointer"
              onClick={() => setActive(c)}
            >
              {/* Receipt area */}
              ...
              <div className="p-5 border-t border-[#E8EAF0]">
                ...
                <div className="flex items-end justify-between gap-2">
                  <div>
                    <p className="text-xs text-[#6B7080] mb-0.5">Withdrawal Amount</p>
                    <p className="text-xl sm:text-2xl font-extrabold text-[#0158c6]" style={{ letterSpacing: '-0.02em' }}>{c.amount}</p>
                  </div>
        ...
        <p className="text-center text-sm text-[#6B7080] mt-10 max-w-2xl mx-auto">
          Names shortened and account numbers masked for privacy. Original receipts available on request from the client concerned.
        </p>
      </div>
        ...
        >
          ...
          >
            ...
            <div className="p-6 flex justify-between items-center border-t border-[#E8EAF0] bg-[#FAFBFD]">
              ...
              <p className="text-2xl font-extrabold text-[#0158c6]">{active.amount}</p>
            </div>
