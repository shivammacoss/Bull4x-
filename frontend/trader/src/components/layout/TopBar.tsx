            ...
            {showMenu && (
              <div className="absolute right-0 top-full mt-2 w-48 py-1 rounded-lg border border-border-glass bg-bg-secondary shadow-lg z-50">
                ...
                <button
                  ...
                  className="flex items-center justify-between w-full px-3 py-2 text-xs text-text-secondary hover:bg-bg-hover hover:text-text-primary transition-fast"
                >
                  ...
                  <span className={`relative w-8 h-[18px] rounded-full transition-colors ${theme === 'dark' ? 'bg-accent' : 'bg-text-tertiary/30'}`}>
                    <span className={`absolute top-[2px] w-[14px] h-[14px] rounded-full bg-white shadow transition-all ${theme === 'dark' ? 'left-[15px]' : 'left-[2px]'}`} />
                  </span>
                </button>
