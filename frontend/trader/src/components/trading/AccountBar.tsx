...
export default function AccountBar() {
  ...
  const items = [
    { label: 'Balance', value: `${sym}${activeAccount.balance.toFixed(2)}` },
    { label: 'Equity', value: `${sym}${equity.toFixed(2)}`, color: equity >= activeAccount.balance ? 'text-buy' : 'text-sell' },
    { label: 'Margin', value: `${sym}${activeAccount.margin_used.toFixed(2)}` },
    { label: 'Free', value: `${sym}${(equity - activeAccount.margin_used).toFixed(2)}` },
    { label: 'Level', value: activeAccount.margin_used > 0 ? `${((equity / activeAccount.margin_used) * 100).toFixed(1)}%` : '--' },
    { label: 'P&L', value: `${totalPnl >= 0 ? '+' : ''}${sym}${Math.abs(totalPnl).toFixed(2)}`, color: totalPnl >= 0 ? 'text-buy' : 'text-sell' },
