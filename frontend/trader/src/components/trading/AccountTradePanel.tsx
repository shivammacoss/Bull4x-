...
export default function AccountTradePanel({ account, onClose }: AccountTradePanelProps) {
  ...
  const freeMargin = Math.max(0, account.free_margin);
      ...
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="lg:w-[340px] shrink-0 border-t border-border-primary">
          ...
          <div className="px-3 py-2 space-y-1 border-t border-border-primary bg-bg-secondary">
            {[
              { label: 'Balance', value: `${currencySymbol(account.currency)}${account.balance.toFixed(2)}` },
              { label: 'Equity', value: `${currencySymbol(account.currency)}${account.equity.toFixed(2)}` },
              { label: 'Margin Used', value: `${currencySymbol(account.currency)}${account.margin_used.toFixed(2)}` },
              { label: 'Free Margin', value: `${currencySymbol(account.currency)}${Math.max(0, account.free_margin).toFixed(2)}`, color: '#2196f3' },
            ].map((r) => (
