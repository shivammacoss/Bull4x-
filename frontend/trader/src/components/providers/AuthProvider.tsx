...
/** All route prefixes that live under the (landing) route group and must be
 *  accessible without authentication. Defined once, used in both the
 *  post-init redirect effect and the pre-init early-return gate. */
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (isInitialized) {
      ...
      if (!isAuthenticated && !isAuthPage && !isPublic) {
        ...
        if (!pathname?.startsWith('/s/')) router.push('/accounts');
      }
    }
  ...
  if (!isInitialized) {
    const isAuthPage = pathname?.startsWith('/auth');
    const isPublicPage = isPublicPath(pathname) || isAuthPage;
