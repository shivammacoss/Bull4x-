...
// P&L / commission are stored in the account currency (INR for INR accounts).
function curSym(currency?: string | null) {
  return (currency || 'USD').toUpperCase() === 'INR' ? '₹' : '$';
}
...
function resolveWsBaseUrl(): string {
  ...
  if (typeof window !== 'undefined') {
    // Safety net: the admin app lives on admin.bull4x.com but the WS
    // route is on the gateway at api.bull4x.com. If the build skipped
    // NEXT_PUBLIC_WS_URL, swap the leading "admin." subdomain to "api."
    // so the Trades page still gets live ticks instead of a dead socket
    // that leaves the Current / Spread columns showing "—".
    const host = window.location.host;
    const apiHost = host.startsWith('admin.') ? 'api.' + host.slice('admin.'.length) : host;
    return `${pageIsHttps ? 'wss:' : 'ws:'}//${apiHost}`;
  }
...
export default function TradesPage() {
  ...
  useEffect(() => {
    // Resolve gateway WS origin. Admin domain (admin.bull4x.com) does NOT
    // proxy /ws/ — that lives on api.bull4x.com — so operators must point
            ...
            <div>
              <div className="overflow-x-auto">
                <table className="w-full min-w-[920px]">
                  <thead>
                    <tr className="border-b border-border-primary bg-bg-tertiary/40">
                      {['Closed', 'User', 'Symbol', 'Side', 'Lots', 'Open', 'Close', 'Comm.', 'Net P&L', 'Reason'].map(c => (
                        <th key={c} className={cn('text-left px-4 py-2.5 text-xxs font-medium text-text-tertiary uppercase tracking-wide', ['Lots', 'Open', 'Close', 'Comm.', 'Net P&L'].includes(c) && 'text-right')}>{c}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {!histLoading && history
                      ...
                      } else if (reason === 'margin' || reason === 'liquidation' || reason === 'margin_call' || reason === 'stop_out') {
                        reasonLabel = 'Stop-out';
                        reasonColor = 'bg-danger/15 text-danger';
                      ...
                      } else if (reason === 'master_deleted') {
                        reasonLabel = 'Master deleted';
                        reasonColor = 'bg-warning/15 text-warning';
                      ...
                      <tr key={t.id} className="border-b border-border-primary/50 transition-fast hover:bg-bg-hover">
                        ...
                        <td className="px-4 py-2.5 text-xs text-text-tertiary text-right font-mono tabular-nums">
                          {(t.commission || 0) > 0 ? `${curSym(t.currency)}${formatMoney(t.commission)}` : '—'}
                        </td>
                        <td className={cn('px-4 py-2.5 text-xs text-right font-mono tabular-nums font-medium', ((t.profit || 0) - (t.commission || 0)) >= 0 ? 'text-success' : 'text-danger')}>
                          {((t.profit || 0) - (t.commission || 0)) >= 0 ? '+' : ''}{curSym(t.currency)}{formatMoney((t.profit || 0) - (t.commission || 0))}
                        </td>
              ...
              {histLoading && <TableSkeleton cols={10} />}
