...
interface WithdrawalBankDetails {
  ...
  // Legacy: older withdrawals stored this under "notes" before the field
  // was repurposed to "bank_name". Kept readable so admin still sees old
  // entries without a backfill migration.
  notes?: string | null;
...
function withdrawalPayoutSummary(w: Withdrawal): string {
  ...
  if (bd.manual) {
    ...
    if (bd.bank_name) parts.push(`Bank: ${bd.bank_name}`);
    // Legacy fallback — pre-rename rows still carry the value under "notes".
    else if (bd.notes) parts.push(String(bd.notes));
...
export default function DepositsPage() {
  ...
  // Payment Limits modal — min/max bounds enforced by the gateway on every
  // deposit / withdrawal submission. Empty string means "not set" in the UI
  // and translates to 0 on save, which the backend treats as no-limit.
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      // Drop the sidebar badge immediately instead of waiting for the 30s poll.
      void usePendingCountsStore.getState().refresh();
    } catch (e: any) {
  ...
  // Fetch the four limit settings from /settings — keys that don't exist
  // simply stay as '' so the inputs render placeholders. The endpoint
  // returns *all* settings; we filter to the four we care about.
  const openLimitsModal = useCallback(async () => {
    setLimitsOpen(true);
    setLimitsLoading(true);
    try {
      const res = await adminApi.get<Array<{ key: string; value: unknown }>>('/settings');
      const list = Array.isArray(res) ? res : [];
      const pick = (k: string) => {
        const row = list.find((s) => s.key === k);
        if (row === undefined || row.value === null || row.value === undefined) return '';
        const n = Number(row.value);
        return Number.isFinite(n) && n > 0 ? String(n) : '';
      };
      setLimits({
        deposit_min: pick('deposit_min'),
        deposit_max: pick('deposit_max'),
        withdrawal_min: pick('withdrawal_min'),
        withdrawal_max: pick('withdrawal_max'),
      });
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'Could not load payment limits');
    } finally {
  ...
  const saveLimits = useCallback(async () => {
    // Validate: min <= max for each pair (when both set).
    ...
    const values = {
      deposit_min: parse(limits.deposit_min),
      deposit_max: parse(limits.deposit_max),
      withdrawal_min: parse(limits.withdrawal_min),
      withdrawal_max: parse(limits.withdrawal_max),
    };
    if (Object.values(values).some((v) => Number.isNaN(v))) {
      toast.error('All limits must be non-negative numbers (or blank).');
      return;
    }
    if (values.deposit_min > 0 && values.deposit_max > 0 && values.deposit_min > values.deposit_max) {
      toast.error('Deposit minimum cannot exceed maximum.');
      return;
    }
    if (values.withdrawal_min > 0 && values.withdrawal_max > 0 && values.withdrawal_min > values.withdrawal_max) {
      toast.error('Withdrawal minimum cannot exceed maximum.');
      return;
    }
    setLimitsSaving(true);
    try {
      await adminApi.put('/settings', { settings: values });
      toast.success('Payment limits saved');
      setLimitsOpen(false);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'Could not save payment limits');
    } finally {
    ...
    <>
      <div className="p-6 space-y-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            type="button"
            onClick={() => void openLimitsModal()}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium border border-border-primary bg-bg-secondary text-text-primary hover:bg-bg-hover transition-fast"
          >
            <Settings size={12} />
            Payment Limits
          </button>
      ...
      {limitsOpen && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget && !limitsSaving) setLimitsOpen(false);
          }}
        >
          <div className="bg-bg-secondary border border-border-primary rounded-md p-5 w-full max-w-md space-y-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h3 className="text-sm font-semibold text-text-primary">Payment Limits</h3>
                <p className="text-xxs text-text-tertiary mt-0.5">
                  Min/max amounts enforced on every deposit & withdrawal. Leave blank for no limit.
                </p>
              </div>
              <button
                type="button"
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <label className="space-y-1">
                    <span className="text-xxs text-text-tertiary block">Deposit min (USD)</span>
                    <input
                      type="number" min="0" step="0.01" inputMode="decimal"
                      value={limits.deposit_min}
                      onChange={(e) => setLimits((p) => ({ ...p, deposit_min: e.target.value }))}
                      placeholder="0"
                      className="w-full px-3 py-2 rounded-md border border-border-primary bg-bg-base text-sm text-text-primary outline-none focus:border-accent/50 font-mono"
                    />
                  </label>
                  <label className="space-y-1">
                    <span className="text-xxs text-text-tertiary block">Deposit max (USD)</span>
                    <input
                      type="number" min="0" step="0.01" inputMode="decimal"
                      value={limits.deposit_max}
                      onChange={(e) => setLimits((p) => ({ ...p, deposit_max: e.target.value }))}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <label className="space-y-1">
                    <span className="text-xxs text-text-tertiary block">Withdrawal min (USD)</span>
                    <input
                      type="number" min="0" step="0.01" inputMode="decimal"
                      value={limits.withdrawal_min}
                      onChange={(e) => setLimits((p) => ({ ...p, withdrawal_min: e.target.value }))}
                      placeholder="0"
                      className="w-full px-3 py-2 rounded-md border border-border-primary bg-bg-base text-sm text-text-primary outline-none focus:border-accent/50 font-mono"
                    />
                  </label>
                  <label className="space-y-1">
                    <span className="text-xxs text-text-tertiary block">Withdrawal max (USD)</span>
                    <input
                      type="number" min="0" step="0.01" inputMode="decimal"
                      value={limits.withdrawal_max}
                      onChange={(e) => setLimits((p) => ({ ...p, withdrawal_max: e.target.value }))}
                      placeholder="No limit"
                      className="w-full px-3 py-2 rounded-md border border-border-primary bg-bg-base text-sm text-text-primary outline-none focus:border-accent/50 font-mono"
                    />
                  </label>
            ...
            <div className="flex justify-end gap-2 pt-1">
              <button
                ...
                onClick={() => setLimitsOpen(false)}
                disabled={limitsSaving}
                className="px-4 py-1.5 rounded-md text-xs font-medium border border-border-primary text-text-secondary hover:bg-bg-hover disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                ...
                className={cn(
                  'px-4 py-1.5 rounded-md text-xs font-semibold inline-flex items-center gap-1.5',
                  'bg-accent text-white hover:bg-accent/90',
                  (limitsLoading || limitsSaving) && 'opacity-50 pointer-events-none',
                )}
              >
                {limitsSaving && <Loader2 size={12} className="animate-spin" />}
                Save
              </button>
            </div>
          </div>
