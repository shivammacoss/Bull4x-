...
/** Format `n` as a signed dollar string with strict sign handling.
 * Fixes the long-standing "+$-0.00" render where JS reported -0 >= 0
 * because Object.is(-0, 0) is false but `-0 >= 0` is true. Only strictly
 * positive numbers get the leading '+', and -0 normalizes to 0. */
...
/** Sign-aware colour class — matches signedDollar so the colour never
 * disagrees with the leading +/- the user reads next to it. */
  ...
  return (
    <div className="bg-bg-secondary border border-border-primary rounded-md p-4">
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        ...
        <div><p className="text-xxs text-text-tertiary">Net P&L</p><p className={cn('font-mono font-medium', signedColor(stats.net_pnl))}>{signedDollar(stats.net_pnl)}</p></div>
      </div>
    </div>
  );
}
    ...
    <>
      <div className="p-6 space-y-4">
        ...
        {data && (
          <div className="bg-bg-secondary border border-border-primary rounded-md">
            <div className="px-4 py-3 border-b border-border-primary">
              <h2 className="text-sm font-medium text-text-primary">Copy Trading / PAMM</h2>
