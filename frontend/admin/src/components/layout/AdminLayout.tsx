...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // Sidebar badge counts: poll once admin is authenticated, refresh every 30s,
  // and on tab focus / visibility. Cheaper than per-row websocket plumbing and
  // good enough for "you have N pending things waiting" UX.
  useEffect(() => {
    if (gate !== 'ready') return;
    const { refresh } = usePendingCountsStore.getState();
    void refresh();
