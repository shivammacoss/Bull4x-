...
class Position(Base):
    ...
    swap = Column(Numeric(18, 8), default=0)
    commission = Column(Numeric(18, 8), default=0)
    # Broker's spread pickup at fill time, computed from the live spread
    # config and frozen here so the close path can copy it into TradeHistory
    # without re-deriving (admin may have changed the rate in between).
...
class Transaction(Base):
    ...
    currency = Column(String(5), server_default="USD", nullable=True)
    fx_rate_applied = Column(Numeric(18, 8), nullable=True)
    converted_amount = Column(Numeric(18, 8), nullable=True)
...
class TradeHistory(Base):
    ...
    swap = Column(Numeric(18, 8), default=0)
    commission = Column(Numeric(18, 8), default=0)
    # Frozen at close — copied straight from Position.spread_revenue so the
    # admin Analytics roll-up has a stable per-trade number even if the
    # spread config changes after the position closes.
    spread_revenue = Column(Numeric(18, 8), default=0)
...
class FxRateConfig(Base):
    """Per-pair broker markup applied to live FX rates shown on UPI deposit/withdraw.

    One row per currency pair (e.g. "USD/INR"). markup_percent is added on top
    of the raw spot rate the gateway pulls from the external FX provider.
    Phase 1 ships with USD/INR at 0% — admin UI to change this comes later.
    """
    __tablename__ = "fx_rate_config"

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    currency_pair = Column(String(16), unique=True, nullable=False, index=True)  # e.g. "USD/INR"
    markup_percent = Column(Numeric(6, 4), nullable=False, default=Decimal("0"), server_default="0")
    updated_at = Column(DateTime(timezone=True), default=datetime.utcnow, onupdate=datetime.utcnow)
