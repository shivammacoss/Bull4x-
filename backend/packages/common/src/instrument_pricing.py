...
async def compute_spread_revenue(
    db: AsyncSession,
    instrument: Instrument,
    lots: Decimal,
    fill_price: Decimal,
    ...
    """Broker's spread pickup on a single fill.

    Formula:
      pips / fixed / variable: spread_value × pip_size × contract_size × lots
      percentage:              notional × (spread_value / 100)

    Uses ``resolve_spread_config``'s priority chain (User > Instrument >
    Segment > Default), the same one the quote widener already consults,
    so the persisted broker pickup matches what the trader actually paid.
    Any lookup failure returns 0 — never abort a fill because the spread
    couldn't be priced.
    """
    try:
        spread_value, spread_type, _pi = await resolve_spread_config(
            db, instrument, user_id=user_id,
        )
    except Exception:
        return Decimal("0")
    ...
    sv = Decimal(str(spread_value))
    st = (spread_type or "pips").lower()
    if st == "percentage":
        notional = Decimal(str(lots)) * Decimal(str(contract_size)) * Decimal(str(fill_price))
        return notional * (sv / Decimal("100"))
    pip_size = instrument.pip_size or Decimal("0.0001")
    return (sv * Decimal(str(pip_size))
            * Decimal(str(contract_size)) * Decimal(str(lots)))
