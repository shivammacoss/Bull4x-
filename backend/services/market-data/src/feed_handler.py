...
class FrozenFeed:
    """No-op feed used when no live price source is available (bad/expired
    Infoway key, network loss) and simulated prices are disabled.

    It produces NO ticks, so the market-data service simply holds the last
    real prices — the stale-quote refresher keeps re-publishing the last mid.
    This replaces the random-walk simulator so a feed outage can NEVER inject
    fake prices that would suddenly swing a user's open-position P&L. Prices
    stay frozen at the last traded price until a real feed resumes.
    """

    async def start(self):
        logger.warning(
            "FrozenFeed active — no live market feed. Holding last traded prices; "
            "no new ticks will be generated until a real feed (Infoway/Corecen) resumes. "
            "Mock/simulated data is intentionally NOT used."
        )
