...
async def list_trade_history(page: int, per_page: int, db: AsyncSession):
    ...
    for t in trades:
        ...
        items.append(TradeHistoryOut(
            ...
            user_email=user_email,
            account_number=account_number,
            currency=(acc.currency or "USD") if acc else "USD",
        ))
...
async def close_position(
    ...
    _adm_cur = (acc.currency if acc else "USD") or "USD"
    _adm_fx = await get_usd_to_account_rate(_adm_cur)
    profit = quote_to_account_pnl(
        ...
        account_currency=_adm_cur,
        usd_inr_rate=_adm_fx,
    )
    ...
    if acc:
        ...
        # Recompute margin_used as SUM of remaining open positions on the
        # account (this position has been marked closed above). Replaces
        # subtractive math that left accounts with MARGIN USED > 0 even when
        # zero open positions remained.
