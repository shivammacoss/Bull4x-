...
def _validate_commission_plan(body: IBCommissionPlanIn) -> None:
    """Reject misconfigurations that would over- or under-pay IBs.

    Without this, an admin could save `[60,40,30,20,10]` and the engine
    would distribute 160% of the commission pool.
    """
    dist = list(body.mlm_distribution or [])
    if any((pct is None or pct < 0) for pct in dist):
        raise HTTPException(status_code=400, detail="mlm_distribution percentages must be ≥ 0")
    total = sum(int(p) for p in dist)
    if total > 100:
        raise HTTPException(
            status_code=400,
            detail=f"mlm_distribution sums to {total}% — must be ≤ 100% (any leftover stays with the platform).",
        )
    if body.mlm_levels is not None and body.mlm_levels > 0 and len(dist) > body.mlm_levels:
        raise HTTPException(
            status_code=400,
            detail=f"mlm_distribution has {len(dist)} levels but mlm_levels is set to {body.mlm_levels}.",
        )
    for fld_name, val in (
        ...
        if val is not None and val < 0:
            raise HTTPException(status_code=400, detail=f"{fld_name} must be ≥ 0")
    if body.spread_share_pct is not None and body.spread_share_pct > 100:
        raise HTTPException(status_code=400, detail="spread_share_pct must be ≤ 100")


async def create_commission_plan(
    body: IBCommissionPlanIn, admin_id: uuid.UUID, ip_address: str | None, db: AsyncSession,
) -> dict:
    _validate_commission_plan(body)
    ...
    for alloc in allocs_q.scalars().all():
        ...
        if investor_acct:
            ...
            for pos in inv_open_q.scalars().all():
                ...
                pos.profit = Decimal("0")
                pos.closed_at = datetime.utcnow()
                # Record so this close shows up in user/admin trade history
                # rather than the position vanishing without a trace.
                db.add(TradeHistory(
                    position_id=pos.id,
