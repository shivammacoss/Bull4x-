...
async def approve_kyc(
    ...
    # Allow approval from any non-approved state. Previously this required
    # status == "submitted", which blocked a legitimate admin override path:
    # when a user re-uploaded after rejection their docs were "pending" but if
    # the re-submission notification was missed and admin acted on the older
    # "rejected" record, the approve call would 400. Idempotent for "approved".
    current = (user.kyc_status or "").lower()
    if current in ("approved", "verified"):
        raise HTTPException(status_code=400, detail="KYC is already approved")

    user.kyc_status = "approved"
    user.updated_at = datetime.utcnow()
    # Clear any prior rejection reasons that might leak into the user's view.
    user.trading_blocked_until = None

    # Approve every still-open document (pending OR previously rejected — admin
    # decision overrides). Already-approved docs are left untouched.
    docs_q = await db.execute(
        select(KYCDocument).where(
            KYCDocument.user_id == user_id,
            KYCDocument.status.in_(["pending", "rejected"]),
        )
    )
