...
async def list_all_deposits(page: int, per_page: int, status: str | None, db: AsyncSession):
    # 'initiated' = an OxaPay payment that was created but never actually paid
    # (e.g. the user pressed Back). These are not real deposits, so they must
    # never appear in the admin history — only completed/started ones do.
    query = select(Deposit).where(Deposit.status != "initiated")
...
async def approve_deposit(
    ...
    # CPA bonus to direct IB — fires only if this is the user's first
    # approved/auto_approved deposit and the plan has cpa_per_deposit > 0.
    try:
        await maybe_pay_cpa_on_deposit(db, deposit=deposit)
    except Exception:
        # CPA failure must never block deposit approval.
        pass
