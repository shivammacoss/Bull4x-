...
def _refresh_account_margin_cols(account) -> None:
    """Recompute equity / free_margin / margin_level after a credit change.

    The b-book matching engine, algo connector and copy engine read the stored
    ``free_margin`` column to approve new trades, and the risk engine only
    refreshes that column for accounts that already hold open positions. So an
    account that is granted credit but holds no position would keep a stale
    ``free_margin`` (computed without the credit) and get its trades rejected.
    Refreshing the columns here makes admin credit usable as margin immediately.
    """
    balance = account.balance or Decimal("0")
    credit = account.credit or Decimal("0")
    margin_used = account.margin_used or Decimal("0")
    account.equity = balance + credit
    account.free_margin = account.equity - margin_used
    account.margin_level = (
        (account.equity / margin_used * Decimal("100")) if margin_used > 0 else Decimal("9999")
    )
...
async def give_credit(
    ...
    _refresh_account_margin_cols(account)
...
async def kill_switch(
    ...
    if account_ids:
        ...
        for pos in positions:
            ...
            # Write a history row so kill-switched positions are still visible
            # in the user's closed-trades list (audit trail + user transparency).
            db.add(TradeHistory(
                position_id=pos.id,
