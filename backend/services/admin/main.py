...
async def _apply_startup_ddl():
    ...
    try:
        async with engine.begin() as conn:
            ...
            # Unique constraint on positions.order_id — prevents duplicate fills
            await conn.execute(text("""
                DO $$ BEGIN
                    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uq_positions_order_id') THEN
                        DELETE FROM positions WHERE id IN (
                            SELECT id FROM (
                                SELECT id, ROW_NUMBER() OVER (PARTITION BY order_id ORDER BY created_at ASC) AS rn
                                FROM positions WHERE order_id IS NOT NULL
                            ) d WHERE rn > 1
                        );
