"""Bull4x Gateway — REST + WebSocket API Server."""
import asyncio
import json
import logging
...
def _audit_oxapay_config() -> None:
    """Loud-fail any half-configured OxaPay state at startup so misconfigs
    surface immediately in logs instead of silently breaking webhooks at
    payment time. We never block startup — OxaPay is optional — but every
    failure mode that would cause "user paid, balance never credited" gets
    a WARNING-level entry the operator can grep for in the gateway logs.
    """
    key = (settings.OXAPAY_MERCHANT_KEY or "").strip()
    callback = (settings.OXAPAY_CALLBACK_BASE_URL or "").strip()
    if not key:
        logger.info("OxaPay disabled — OXAPAY_MERCHANT_KEY is empty.")
        return
    if settings.OXAPAY_SANDBOX:
        logger.warning("OxaPay running in SANDBOX mode — merchant key sent as 'sandbox'.")
    if not callback:
        logger.error(
            "OxaPay misconfigured: OXAPAY_CALLBACK_BASE_URL is empty. "
            "OxaPay will accept payments but the webhook callback will 404, "
            "deposits will never auto-credit. Set this to a publicly reachable "
            "https URL (e.g. https://api.bull4x.com)."
        )
        return
    if callback.startswith("http://localhost") or "127.0.0.1" in callback:
        logger.error(
            "OxaPay misconfigured: OXAPAY_CALLBACK_BASE_URL points at localhost (%s). "
            "OxaPay's servers cannot reach localhost — webhooks will never arrive. "
            "Use a public https URL.", callback,
        )
    elif callback.startswith("http://"):
        logger.warning(
            "OxaPay: OXAPAY_CALLBACK_BASE_URL is http:// (%s). Production should be https://.",
            callback,
        )
    else:
