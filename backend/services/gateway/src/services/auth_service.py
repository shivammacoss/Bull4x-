...
DEMO_SHARED_EMAIL = "demo@bull4x.com"
...
# ─── Forgot / Reset password via OTP (mobile app) ───────────────────────────

async def forgot_password_otp(email: str, request: Request, db: AsyncSession) -> dict:
    """Mobile app flow: email a 6-digit OTP instead of a reset link.
    Always returns the same response to avoid account enumeration."""
    rate_limit_http(request, "forgot-password-otp", 5, 600.0)
    resp = {
    ...
    from packages.common.src.smtp_mail import send_password_reset_otp, smtp_configured
    if smtp_configured():
        sent = await send_password_reset_otp(user.email, otp)
        if sent:
            logger.info("Password reset OTP sent to %s", user.email)
        else:
            logger.error("Password reset OTP email failed for %s", user.email)
    elif get_settings().ENVIRONMENT == "development":
        logger.warning("Password reset OTP (dev, SMTP not configured) for %s: %s", user.email, otp)
    else:
        logger.warning("SMTP not configured — no OTP sent for %s", user.email)

    return resp


async def verify_reset_otp(email: str, otp: str, new_password: str, request: Request, db: AsyncSession) -> dict:
    """Mobile app flow: verify the OTP (scoped to the user) and set a new password.
    Returns {success: bool, message: str} with HTTP 200 so the app can show the message."""
    rate_limit_http(request, "verify-reset-otp", 20, 600.0)
    email = (email or "").strip().lower()
    otp = (otp or "").strip()

    invalid = {"success": False, "message": "Invalid or expired verification code."}
    result = await db.execute(
        select(User).where(func.lower(User.email) == email)
    )
    user = result.scalar_one_or_none()
...
async def setup_2fa(user_id: UUID, db: AsyncSession) -> dict:
    ...
    totp = pyotp.TOTP(secret)
    provisioning_uri = totp.provisioning_uri(name=user.email, issuer_name="Bull4x")
