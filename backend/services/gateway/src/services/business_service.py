...
async def claim_payout(user_id: UUID, account_id: UUID | None, db: AsyncSession) -> dict:
    """Let an IB claim their pending commissions into a trading account."""
    ...
    if not profile.is_active:
        raise HTTPException(status_code=403, detail="IB profile is inactive")
    if (profile.pending_payout or 0) <= 0:
        raise HTTPException(status_code=400, detail="No pending payout to claim")

    result = await settle_pending_commissions(
        db, ib_user_id=user_id, target_account_id=account_id,
    )
    if result.get("error") == "no_account":
        raise HTTPException(
            status_code=400,
            detail="No active live trading account found. Create a live account first.",
        )
    await db.commit()
    return result


async def get_ib_accounts(user_id: UUID, db: AsyncSession) -> list[dict]:
    """Return the IB's live trading accounts for payout target selection."""
    result = await db.execute(
    ...
    return [
        {
            "id": str(a.id),
            "login": a.login,
            "label": a.label or f"Account {a.login}",
