...
async def create_payment(
    ...
    payload: dict = {
        ...
        "lifeTime": 30,  # minutes
        # 1 = payer covers OxaPay's commission so the MERCHANT receives the full
        # invoice amount. With 0 the fee was deducted from the payment, so a $100
        # request settled as ~$98-99 net — the user's deposit looked $1-2 short of
        # what they entered. Keeping the payer on the fee makes "amount entered" ==
        # "amount received" == "amount credited".
        "feePaidByPayer": 1,
    }
...
def verify_webhook_signature(raw_body: bytes, received_hmac: str) -> bool:
    ...
    Returns False (rejects the webhook) when the merchant key isn't
    configured — otherwise an empty key would let an attacker who knows
    the empty-string HMAC bypass auth. Misconfigured deployments should
    visibly fail at the gateway log level instead of silently accepting.
    """
    settings = get_settings()
    raw_key = (settings.OXAPAY_MERCHANT_KEY or "").strip()
    if not raw_key:
        logger.error(
            "Rejecting OxaPay webhook — OXAPAY_MERCHANT_KEY is not set. "
            "No webhook can be verified without it."
        )
        return False
    if not received_hmac:
        return False
    computed = hmac.new(raw_key.encode(), raw_body, hashlib.sha512).hexdigest()
