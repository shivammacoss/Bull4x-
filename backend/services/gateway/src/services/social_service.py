...
async def refill_allocation(
    allocation_id: UUID, amount: Decimal, user_id: UUID, db: AsyncSession,
) -> dict:
    """Top up an existing follower allocation with additional capital.

    Unlike `start_copy` (which only handles first-time follow), this just
    adds money to the existing investor sub-account so the user can size up
    a strategy that's already live. No minimum-investment check — a
    follower already past the floor on signup can top up any amount > 0.

    For MAM / signal-provider masters the deposit is also recorded against
    the active monthly settlement period via `record_capital_event` so the
    period P&L formula correctly subtracts the topped-up capital from
    "profit" at settlement time.
    """
    if amount <= 0:
    ...
    investor_acct = await db.get(TradingAccount, allocation.investor_account_id) if allocation.investor_account_id else None
    if not investor_acct:
        raise HTTPException(status_code=400, detail="Subscription account missing")

    user.main_wallet_balance = wallet_bal - amount
    investor_acct.balance = (investor_acct.balance or Decimal("0")) + amount
    investor_acct.equity = investor_acct.balance + (investor_acct.credit or Decimal("0"))
    investor_acct.free_margin = investor_acct.equity - (investor_acct.margin_used or Decimal("0"))

    allocation.allocation_amount = (allocation.allocation_amount or Decimal("0")) + amount

    db.add(Transaction(
        user_id=user_id,
        account_id=investor_acct.id,
        type="withdrawal",
        amount=-amount,
        description=f"Refill subscription → account {investor_acct.account_number}",
    ))

    # MAM/signal monthly settlement: register the deposit so future settlement
    # doesn't mistake topped-up capital for trading profit.
    master_q = await db.execute(select(MasterAccount).where(MasterAccount.id == allocation.master_id))
    master = master_q.scalar_one_or_none()
    if master and master.master_type in ("mamm", "signal_provider"):
        try:
            from packages.common.src.mam_settlement_service import record_capital_event
            await record_capital_event(db, allocation.id, "deposit", amount)
        except Exception:
            # non-fatal — settlement period accounting will self-heal at settlement
            pass

    await db.commit()
    await db.refresh(allocation)
    await db.refresh(investor_acct)
    await db.refresh(user)

    return {
        "allocation_id": str(allocation.id),
        "investor_account": investor_acct.account_number,
        "added": float(amount),
        "new_balance": float(investor_acct.balance),
        "new_allocation_amount": float(allocation.allocation_amount),
        "wallet_balance": float(user.main_wallet_balance),
        "message": f"Added ${float(amount):.2f} to subscription",
    }
        ...
        if alloc.investor_account_id:
            ...
            for pos in open_positions:
                ...
                try:
                    ...
                    _inv_cur = (inv_acct.currency if inv_acct else "USD") or "USD"
                    _inv_fx = await get_usd_to_account_rate(_inv_cur)
                    pnl = quote_to_account_pnl(
                        ...
                        account_currency=_inv_cur,
                        usd_inr_rate=_inv_fx,
                    )
                except Exception:
                    pass
                unrealized_pnl += pnl
...
async def stop_copy(allocation_id: UUID, user_id: UUID, db: AsyncSession) -> dict:
    ...
    for copy in open_copies:
        ...
        from packages.common.src.fx_utils import get_usd_to_account_rate
        _sc_acct = await db.get(TradingAccount, investor_pos.account_id)
        _sc_cur = (_sc_acct.currency if _sc_acct else "USD") or "USD"
        _sc_fx = await get_usd_to_account_rate(_sc_cur)
        ...
        db.add(TradeHistory(
            ...
            spread_revenue=investor_pos.spread_revenue or Decimal("0"),
            profit=net,
...
async def withdraw_managed_account(
    ...
    for copy in open_copies:
        ...
        from packages.common.src.trading_service import quote_to_account_pnl
        from packages.common.src.fx_utils import get_usd_to_account_rate
        _p_acct = await db.get(TradingAccount, investor_pos.account_id)
        _p_cur = (_p_acct.currency if _p_acct else "USD") or "USD"
        _p_fx = await get_usd_to_account_rate(_p_cur)
        gross = quote_to_account_pnl(
            gross,
            getattr(instrument, "base_currency", None),
            getattr(instrument, "quote_currency", None),
...
async def get_my_followers(user_id: UUID, db: AsyncSession) -> dict:
    ...
    followers = []
    total_lifetime_commission = 0.0
    total_current_month_pending = 0.0
    for allocation, user, account in allocations:
        ...
        # Lifetime commission already paid out from this follower (settled
        # periods). Current month's pending = what would settle if the active
        # period closed right now — drives the master's "this month" view.
        lifetime_commission = float(allocation.lifetime_master_fee_paid or 0)
        current_month_pending = 0.0
        try:
            pending = await compute_pending_fee(db, allocation.id)
            current_month_pending = float(pending.get("pending_master_share", 0))
        except Exception:
            current_month_pending = 0.0

        total_lifetime_commission += lifetime_commission
        total_current_month_pending += current_month_pending
        ...
        followers.append({
            ...
            "current_month_pending_commission": current_month_pending,
        })
    ...
    return {
        ...
        "lifetime_commission_earned": float(master.total_fee_earned or 0),
        "current_month_pending_commission": total_current_month_pending,
        "followers": followers,
    }
