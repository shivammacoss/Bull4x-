...
async def _get_user_accounts(
    ...
    """Return the user's trading accounts, optionally filtered by demo/live.

    `is_demo=True`  → only demo accounts
    `is_demo=False` → only live accounts
    `is_demo=None`  → both (legacy default)

    The portfolio API exposes this filter so the UI can keep demo and live
    activity in separate views (per user request — demo trades must NOT
    aggregate into the same totals as real trades).
    """
    q = select(TradingAccount).where(TradingAccount.user_id == user_id)
    if is_demo is not None:
        q = q.where(TradingAccount.is_demo == bool(is_demo))
...
def _compute_pnl(pos: Position, current_price: Decimal,
                  account_currency: str = "USD", usd_inr_rate: Decimal | None = None) -> Decimal:
...
async def portfolio_summary(
    user_id: UUID, account_id: UUID | None, db: AsyncSession,
    is_demo: bool | None = None,
) -> dict:
    accounts = await _get_user_accounts(user_id, db, is_demo=is_demo)
    if not accounts:
        return {
    ...
    for a in accounts:
        ...
        acct_currency_map[a.id] = cur
        if cur not in fx_rate_cache:
            fx_rate_cache[cur] = await get_usd_to_account_rate(cur)
    ...
    for pos in open_positions:
        ...
        pos_currency = acct_currency_map.get(pos.account_id, "USD")
        pos_fx_rate = fx_rate_cache.get(pos_currency)
        prices = await _get_current_price(symbol)
        if prices:
            bid, ask = prices
            cp = bid if (pos.side == OrderSide.BUY or pos.side.value == "buy") else ask
            pnl = _compute_pnl(pos, cp, account_currency=pos_currency, usd_inr_rate=pos_fx_rate)
        else:
...
async def trade_history(
    ...
    acct_number_by_id = {a.id: a.account_number for a in accounts}
    # account_id → currency so each row can render its own symbol (₹ vs $).
    # TradeHistory.profit/commission are stored in the account currency.
    acct_currency_by_id = {a.id: (a.currency or "USD") for a in accounts}
    ...
    for t in trades:
        ...
        items.append({
            ...
            "currency": acct_currency_by_id.get(t.account_id, "USD"),
