...
class CopyTradeEngine:
    ...
    async def _open_copy(
        ...
        available_margin = (
            (investor_account.balance or Decimal("0"))
            + (investor_account.credit or Decimal("0"))
            - (investor_account.margin_used or Decimal("0"))
        )
        ...
        # Per-lot commission lookup — copy mirrors used to hard-code 0 here,
        # so followers traded for free and broker analytics showed near-zero
        # revenue even with heavy copy volume. resolve_commission walks the
        # ChargeConfig priority chain (User > Instrument > Segment > Default).
        # A lookup failure must not abort the mirror; default to 0 in that case.
        copy_lots_dec = Decimal(str(copy_lots))
        try:
            commission = await resolve_commission(
                db, instrument, copy_lots_dec, master_pos.open_price,
                user_id=investor_account.user_id,
            )
        except Exception as e:
            logger.warning("resolve_commission failed for copy mirror: %s", e)
        ...
        try:
            ...
            logger.warning("compute_spread_revenue failed for copy mirror: %s", e)
        ...
        # Debit the per-lot commission from the follower's balance — same
        # accounting the manual order path performs in trading_service.place_order.
        if commission and commission > 0:
            investor_account.balance = (investor_account.balance or Decimal("0")) - commission
            investor_account.equity = (investor_account.balance or Decimal("0")) + (investor_account.credit or Decimal("0"))
        investor_account.free_margin = investor_account.equity - investor_account.margin_used

        # IB commission is NOT distributed on OPEN — it fires when the
        # mirrored position closes (see _close_copy below). Paying on open
        # would let a master-investor pair open/close to mint commission.
    ...
    async def _close_copy(self, copy: CopyTrade, master: MasterAccount, db: AsyncSession):
        ...
        inv_acct_for_cur = await db.get(TradingAccount, investor_pos.account_id)
        inv_currency = (inv_acct_for_cur.currency if inv_acct_for_cur else "USD") or "USD"
        inv_fx_rate = await get_usd_to_account_rate(inv_currency)
        gross_profit = quote_to_account_pnl(
            ...
            account_currency=inv_currency,
            usd_inr_rate=inv_fx_rate,
        )
        ...
        if investor_account:
            ...
            # Recompute margin_used as the SUM of all remaining open positions
            # (the just-closed mirror has status='closed' from line above) —
            # subtractive math drifted over time and stranded margin.
        ...
        db.add(history)
        await db.flush()  # populate history.id for IB idempotency key

        # IB commission for this investor's mirrored close (skip demo, walks
        # the investor's referrer chain inside the engine).
        if investor_account and investor_account.user_id:
            try:
                ...
                await distribute_ib_commission(
                    db,
                    trader_user_id=investor_account.user_id,
                    source_trade_id=history.id,
                    lots=investor_pos.lots,
                    instrument_symbol=(investor_pos.instrument.symbol if investor_pos.instrument else "?"),
                    is_demo=bool(investor_account.is_demo),
                    trader_commission_charged=(investor_pos.commission or Decimal("0")),
                )
            except Exception as exc:
                logger.error(
                    "IB commission distribute failed on copy close investor=%s pos=%s: %s",
                    investor_account.id, investor_pos.id, exc,
                )
