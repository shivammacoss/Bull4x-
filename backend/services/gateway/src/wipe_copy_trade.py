...
async def _close_open_copy_positions(allocation, db, execute: bool) -> Decimal:
    ...
    for copy in open_copies:
        ...
        _w_acct = await db.get(TradingAccount, pos.account_id)
        _w_cur = (_w_acct.currency if _w_acct else "USD") or "USD"
        _w_fx = await get_usd_to_account_rate(_w_cur)
