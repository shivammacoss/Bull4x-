...
@router.post("/ib/claim-payout")
async def claim_payout(
    body: ClaimPayoutRequest = ClaimPayoutRequest(),
    current_user: dict = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    return await business_service.claim_payout(
        user_id=current_user["user_id"],
...
@router.get("/ib/accounts")
async def ib_accounts(
    current_user: dict = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    return await business_service.get_ib_accounts(
