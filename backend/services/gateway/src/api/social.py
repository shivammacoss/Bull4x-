...
@router.post("/refill/{allocation_id}")
async def refill_allocation(
    ...
    """Top up an existing subscription with additional capital — no minimum."""
    return await social_service.refill_allocation(
        allocation_id=allocation_id, amount=amount,
        user_id=current_user["user_id"], db=db,
    )
