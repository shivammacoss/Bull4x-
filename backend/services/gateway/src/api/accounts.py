...
async def list_openable_account_groups(
    ...
    `is_demo` query param explicitly picks demo-vs-live groups so the UI can
    show separate "Live Accounts" and "Demo Accounts" sections. Omit it to
    fall back to legacy behaviour (auto-filter by user's demo flag).
    """
...
async def delete_trading_account(
    ...
    """Permanently remove a trading account (live or demo) owned by the user."""
