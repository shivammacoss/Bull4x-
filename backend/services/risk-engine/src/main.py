...
class RiskEngine:
    ...
    async def _margin_monitor(self):
        ...
        while self._running:
            try:
                async with AsyncSessionLocal() as db:
                    ...
                    for account in accounts:
                        ...
                        if not positions:
                            # No open positions but margin_used > 0 → phantom
                            # margin left behind by a drifting close/stop-out
                            # (e.g. INR reserve released as USD). Zero it so Free
                            # Margin recovers and new trades aren't blocked. This
                            # also self-heals any account already stuck this way.
                            if account.margin_used and account.margin_used > 0:
                                ...
                                logger.info(
                                    "Cleared phantom margin on %s (no open positions)",
            ...
            # Tighter loop → stop-out fires sooner on fast-moving, high-leverage
            # symbols (e.g. XAUUSD), shrinking the window where an account can
            # blow past the stop-out level. NBP in _execute_stop_out covers any
            # residual gap.
    ...
    async def _execute_stop_out(self, account: TradingAccount, positions: list[Position], db: AsyncSession):
        ...
        for pos in sorted_positions:
            ...
            pos.comment = (pos.comment or "") + " | Stop-out auto-close"
            ...
            # Persist a TradeHistory row so this close shows up in the user's
            # closed-trades list and the admin trades view. Without this, the
            # position silently disappears from history.
            db.add(TradeHistory(
                position_id=pos.id,
            ...
            # LP routing disabled — stop-out closes stay on internal b-book engine.
        ...
        # Authoritatively rebuild margin_used from the positions STILL open on
        # this account (zero after a full stop-out). The per-position release in
        # the loop above drifts for INR accounts — margin was reserved in INR
        # but released using a USD figure — leaving phantom "Used Margin" and a
        # negative Free Margin that then blocks new trades ("insufficient
        # margin"). Recomputing from open positions sets it correctly to 0.
        try:
            from packages.common.src.trading_service import recompute_account_margin
            await recompute_account_margin(db, account)
        except Exception as exc:
            logger.warning("Stop-out margin recompute failed for %s: %s", account.account_number, exc)

        # ── Negative balance protection ──────────────────────────────────
        # A fast/gapping market can realise a loss larger than the remaining
        # equity between two 1s margin checks, pushing balance below zero. A
        # retail trader must never owe the broker, so clamp to zero and record
        # the absorbed deficit as an adjustment. This is what stops the trading
        # account from ever going into minus.
        if account.balance < Decimal("0"):
            ...
            db.add(Transaction(
                ...
                account_id=account.id,
                type="adjustment",
                amount=deficit,  # positive credit that brings balance back to 0
                balance_after=Decimal("0"),
                description=f"Negative balance protection — broker absorbed {deficit:.2f}",
            ))
            ...
            logger.warning(
                "Negative balance protection applied to account %s — absorbed %.2f",
