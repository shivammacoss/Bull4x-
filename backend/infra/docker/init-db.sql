...
-- Seed super admin. The bcrypt hash below is the legacy hash carried over from the
-- previous brand seed; it decodes to a temporary password and MUST be rotated.
-- The Alembic migration 0002 overwrites this with bcrypt(ADMIN_PASSWORD from .env) —
-- recommended login after migrations:
--   email:    admin@bull4x.com
--   password: Bull4xAdmin2025!  (after `docker compose --profile migrate up migrate`)
-- Always rotate the password via the admin UI after first login.
INSERT INTO users (email, password_hash, first_name, last_name, role, status, kyc_status)
VALUES ('admin@bull4x.com', '$2b$12$DVZYBxBsibEVVo.zd/5HF.cow4fTmbhH6RZHfDXluZfPyEjKQDhPm', 'Super', 'Admin', 'super_admin', 'active', 'approved');
